...
- **中栏**：消息时间线（text / thinking / toolCall / toolResult 各类块）。可以在最早和最新的
  200 条之间切换，也可以只看 **user**（人的话）、**assistant**（模型的话）或 **tools**（工具调用
  与结果）——三者是互不重叠的类别，而不是按角色字段判定，所以工具往来不会再混进发言者里。
...
手机上三栏变成一次一屏，用 **Sessions / Conversation / Details** 切换——会话内容不会再被
一个要先滚过去的列表挤到下面；点列表里的会话会直接跳过去。页面每 10 秒刷新一次，且不会打断你
...
| `/sessions/<pattern>` | 是 | 单个会话；可接 `/messages`、`/final` 或 `/export` |
