...
折叠的分组）刷新后仍然保留。`/ui#<sessionId>` 可作为深链接。明暗主题跟随系统，页头的半圆按钮一键切换；切回与系统一致的主题时，会重新交还给系统跟随。

左右两栏各有自己的标题栏，放着只作用于这一栏的控件：项目分组的"全部折叠"，以及栏边缘的折叠箭头，点一下就把这一栏收起，把整个宽度让给会话内容。收起后留一条窄栏，箭头指回来；折叠状态会被记住。
